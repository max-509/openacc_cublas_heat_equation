...
#pragma acc data create(err)
    {
#pragma acc host_data use_device(buff_grid, diff_buff, err)
      {
        CUBLAS_CHECK(cublasCopy(handle, grid_sqr, buff_grid + grid_sqr, 1, diff_buff, 1));
        CUBLAS_CHECK(cublasAxpy(handle, grid_sqr, &a, buff_grid, 1, diff_buff, 1));
        CUBLAS_CHECK(cublasIamax(handle, grid_sqr, diff_buff, 1, &err_idx));

        CUBLAS_CHECK(cublasCopy(handle, 1, diff_buff + err_idx, 1, &err, 1));
      }
...
{
  return "BLAS without error copying";
}
